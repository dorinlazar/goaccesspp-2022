...
int parse_conf_file(int* argc, char*** argv) {
  ...
  append_to_argv(&nargc, &nargv, xstrdup(*argv[0] ?: "goaccesspp"));
...
static char* append_spec_date_format(const char* date_format, const char* spec_format) {
  char* s = (char*)xmalloc(snprintf(NULL, 0, "%s%s", date_format, spec_format) + 1);
...
static void set_spec_date_time_format(void) {
  ...
  flen = (strlen(fmt) * 2) + 1;
  buf = (char*)xcalloc(flen, sizeof(char));
...
void GConf::PushUniqueToArray(const std::string& value, std::vector<std::string>& arr) {
